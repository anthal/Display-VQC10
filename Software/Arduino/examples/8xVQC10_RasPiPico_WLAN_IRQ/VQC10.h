...
template <uint8_t n_digits = 32> class VQC10 {
public:
  struct Pins {
  ...
  void tick() {
    ...
    for (uint8_t digit = 0; digit < n_digits; digit++) {
      ...
      bit4 = bitRead(digit, 4);  //
      //Serial.printf("Digit: %1d: Bits: %1d %1d %1d %1d %1d\n", digit, bit4, bit3, bit2, bit1, bit0);
